...
@Component({
  ...
  templateUrl: './staniste.page.html',
  styleUrls: ['./staniste.page.scss'],
})
export class StanistePage implements OnInit, OnDestroy {
  showImage = false;
  showInfo = false; 
  showError = false;
  ...
  touchStart(event: TouchEvent, imageId: string) {
    console.log('TOUCH START TRIGGERED ON staniste:', imageId);
  ...
  touchMove(event: TouchEvent) {
    console.log("staniste touchMove",Touch)
    const touch = event.touches[0];
    if (touch) {
      event.preventDefault();
  ...
  nextPage() {
    this.stopBackgroundMusic();
    this.router.navigateByUrl('/gradnja', { replaceUrl: true });
  }
}
