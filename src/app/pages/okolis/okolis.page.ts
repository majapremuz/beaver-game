...
export class OkolisPage implements OnInit, OnDestroy {
  showImage = false;
  showInfo = false; 
  showError = false;
...
addTouchListeners() {
  ...
  images.forEach((img: Element) => {
    ...
    element.addEventListener('touchstart', (e) => this.touchStart(e as TouchEvent, element.id), {
      passive: false
    });
    element.addEventListener('touchmove', (e) => this.touchMove(e as TouchEvent), {
      passive: false
    });
    element.addEventListener('touchend', (e) => this.touchEnd(e as TouchEvent, element.id), {
      passive: false
    });
  });
}
  ...
  nextPage() {
    this.stopBackgroundMusic();
    this.router.navigateByUrl('/zastita', { replaceUrl: true });
  }
}
